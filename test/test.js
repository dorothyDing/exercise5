...
describe('regex', function() {
    it('可匹配 LITERALLY 开始', function() {
        ...
        assert.ok(!anchor_start(`FIGURATIVELY can't even`),
            `FIGURATIVELY can't even`
        )
        assert.ok(!anchor_start(`I am LITERALLY on a roll down a hill right now`),
            `I am LITERALLY on a roll down a hill right now`
        )
        assert.ok(!anchor_start(`literaturely`),
            `literaturely`
        )
        assert.ok(!anchor_start(`litERALLY whatever`),
            `litERALLY whatever`
        )
        assert.ok(
    ...
    it('可匹配 BANANAS 结束', function() {
        ...
        assert.ok(!anchor_end(`BANANAS ARE GREEN OR YELLOW`),
            `BANANAS ARE GREEN OR YELLOW`
        )
        assert.ok(!anchor_end(`NOTHING`),
            `NOTHING`
        )
        assert.ok(!anchor_end(`EVERYTHING IS BANANAS.`),
    ...
    it('首字母必须满足下面条件', function() {
        ...
        assert.ok(!char_class(`babbage`),
            `babbage`
        )
        assert.ok(!char_class(`AAA`),
            `AAA`
        )
        assert.ok(
            char_class(`123`),
    ...
    it('匹配开头非数字，第二个字符非大写字母', function() {
        ...
        assert.ok(!negated_char_class(`1A`),
            `1A`
        )
        assert.ok(
        ...
        assert.ok(!negated_char_class(`ABC`),
            `ABC`
        )
        assert.ok(
        ...
        assert.ok(!negated_char_class(`555`),
            `555`
        )
        assert.ok(!negated_char_class(`5z`),
            `5z`
        )
        assert.ok(
            negated_char_class(`...`),
